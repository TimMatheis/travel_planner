...
def get_category_data(test_set, user_activity):
    """
    Converts the monthly observations to seasonal observations by grouping all categorical variables and taking the mean of the numeric variables.
    Further, the function filters the dataset to only allow for cities which are compatible with the type of holiday indicated by the user.

    Returns:
        data frame with one row of seasonal variables for each suitable city
    """
